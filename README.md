...
Эти объекты в облаке нам предстоит создать. 
...
Нам понадобится для этого данные нашего сервисного акканта и данне о регионе YC в катором мы работаем
...
В этой колонке будут храниться записи о блокировке состояния нашего стейта. 
...
Манифест создает  bucket, в которое Terraform будет сохранять состояние инфраструктуры
...
Из блок outputs возьмем значение Document API эндпоинт, он пригодиться на следующем шаге. 
...
Мне показалось интересным кейсом попробовать его в работе, тем более что гранта выданого в начале обучения осталось достаточно и можно так сказать на последок шикануть. 
...
В создание кластера сервиса Yandex Managed Service for Kubernetes нет какой сложности, информации достаточно много и документации.

Так же в данном блоке происходит создание backend для хранения стейт файла нашего терраформа. 
...
В данном манифесте описано создание  backend для хранения state файла терраформ.

Для выполнения локального подключения используется уже настроенный предварительно aws профиль. 

В данном блоке нет возможности использовать переменные, что в дальнейшем привело к некоторым сложностям, расскажу об этом ниже. 

Так же в данном блоке необходимо заменить аргумент dynamodb, но тот что мы получили в результате предыдущего шага. 

                 Из блока outputs возьмем значение Document API эндпоинт. 
...
Запускаем создание нашего кластера.
...
Наш образ собран и готов к работе. 

Создадим необходимые манифесты для деплоя в кубер
...
Для создания мониторинга выберем установку с помощью helm чарта. 
Для сервиса так же будем использовать NodePort
...
Вишенкой на торте нашего проекта, является задания настройки автоматизации CI и CD наших инфтраструктуры и приложения. 
...
Абсалютно все задачи CI-CD я решил выполнить с его помощью, мне показалось, что это будет более локонично. 
Хранение репозиториев, контроль версий, CI и CD, все в одном месте. 

Для организации подобных задач в первую очереь не обходимо настроить secret для доступа ко всем используемым нами сервисам. 

Таких оказало не мало.

Но пойдем по порядку. 

Создадим в нашем репозитории каталог .github/workflows
В нем хранятся все workflows манифесты описанные на языкt yaml.

Создадим наш первый workflows, который будет нам собирать наше приложение и пушить его в docker hub. 

### action-build.ym

Данный workflows сработает при пуше в ветку dev в каталог app/
```
...
Перейдем на вкладку с серетами в нашем репозитории в создадим ненужные сикреты.
...
Выполним изменения в нашей папке app/ и проверим работы сборки и пуша нашего приложения. 

В файле index.py изменим название шапки сайта  REAL на NEREAL
...
Проверим новый образ, выполним и проверим наше приложение. 
```
...
Теперь настроим workflow который будет пересоздавать наше приложение в кубер кластере.

Данный манифест выполнит пересоздание подов когда запушат изминения в main ветку в каталог app
...
Заполним серкреты в Git
![image](https://github.com/user-attachments/assets/29ed7719-205c-40d3-a4e2-d3a566529e3d)

Добавим наш workflow к проекту и запушем изминение в main ветку. 

Для наглядности изменим в шапке нашего сайта NEREAL на DEPLOY_V_KUBER

Мы внесли изминения в файлах приложения и за пушили их в dev ветку, у нас выполнился workflow сборки и доставки в репозиторий. 

![image](https://github.com/user-attachments/assets/0869ddde-e53b-4051-a5ae-f2f2fa2a9a55)

Теперь с мерджим изменения в main ветку и проверим как отработает наш новый workflow.

workflow ожидаемо выполнились. 
...
```
Для корректного выполнения подключения к нашему backend файлу стейта, буду использовать опять жу secret и ключи при выполнении команд. 

Напомню что в блоке backend переменные использовать нельзя. 

Дополним наши секреты:
...
- BACKEND_KEY - ключ файла хранения стейта
...
Видим наши изменения, и мерджим в main
